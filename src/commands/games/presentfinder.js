...
module.exports = {
  ...
  run: async (ctx) => {
    ...
    let trees = ctx.interaction.options.getNumber('trees') || randInt(...minMax);
    ...
    const msg = await ctx.interaction.editReply({
      embeds: [embed], files: [{ attachment: canvas.toBuffer(), name: 'presentfinder.png' }],
      components: Array.from({ length: Math.ceil(trees / 5) }, (_, ri) => ({
        type: 1,
        components: Array.from({
          length: Math.ceil(trees / 5) === 1 ? trees : (Math.ceil(trees / 5) === ri + 1) ? trees - (Math.ceil(trees / 5) - 1) * 5 : 5
        }, (_, i) => ({ type: 2, style: 2, emoji: { name: '🎄' }, customId: `presentfinder_${ri}_${i + 1}` })).slice(0, 5)
      }))
    });

    const timeLeft = Date.now() + 120000;
    const collector = msg.createMessageComponentCollector({
      filter: (inter) => inter.user.id === ctx.interaction.user.id && inter.customId.startsWith('presentfinder_'),
      time: 120_000
    })
    ...
    collector.on('end', (_, reason) => {
      ...
      if (reason === 'fail') embed2 = new MessageEmbed()
        .setTitle('Yikes').setColor('RED')
      ...
      else if (reason === 'success') embed2 = new MessageEmbed()
        .setTitle('Nice job').setColor('GREEN')
